		...
		<div class="shell">
			...
			<div class="main">
				...
				<section class="slider-section">
					<table style="width:1100px;"><tr><td valign="top" align="justify" style="width:300px;">
						<h2 class="alignleft"><u><h3><strong>Message Search</strong></h3></u></h2>
					<div class="cl">&nbsp;</div>
...
-->						
						...
						$year = $_POST['year'];
							...
							if($year!="" && $month!="" && $day!="" && $name2!="")
							{
								...
								$sql2 = "SELECT * FROM messages WHERE token IN (SELECT token FROM messages WHERE speaker='$name2' AND day='$day' AND month='$month' AND year='$year') AND year='$year' AND  day='$day' AND month='$month'" ;
								$res2 = $db->query($sql2);
								$i=0;
							...
							else
							{
								...
								while($res = $res2->fetchArray(SQLITE3_ASSOC)){
									$i=$res['token'];
									//echo "yo";
									echo '<h4>Date : '.$res['day'].'/'.$res['month'].'/'.$res['year'].'</br></h4>';
									echo '<h3>'.$res['speaker'].' : '.$res['dialogue'].'</h3></br>';
									}
							}
